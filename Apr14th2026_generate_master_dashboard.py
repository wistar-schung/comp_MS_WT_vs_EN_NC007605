...
ROS_CURATED = ["SOD1", "SOD2", "SOD3", "CAT", "PRDX1", "PRDX2", "PRDX6", "GPX1", "GPX4", "GSR", "TXN", "TXNRD1", "NFE2L2", "HMOX1", "NQO1"]
...
score_if_valid(adata, ROS_CURATED, 'ROS_Curated_Score')
...
cols_to_copy = [
    ...
    'ABC_Score', 'T_Exhaustion_Score', 'Mono_Inflammatory_Score', 'IFN_Score', 'ROS_Curated_Score', 
...
fig_ct = px.scatter(
    plot_df_umap, x='UMAP1', y='UMAP2', color='majority_voting', facet_col='dataset', 
    render_mode='webgl', title="UMAP: Cell Types (WT vs Enriched)",
    color_discrete_map=CELLTYPE_COLOR_MAP, category_orders={'majority_voting': original_cell_types}
)
export_plot_and_table(fig_ct, sum_cat(plot_df_umap, 'majority_voting'), DIR_GLOBAL, "02_CellTypes.html", "Cell Types by Dataset")

fig_ros_global = px.scatter(plot_df_umap, x='UMAP1', y='UMAP2', color='ROS_Curated_Score', facet_col='dataset', hover_data=hover_cols_global, color_continuous_scale='Greens', render_mode='webgl', title="UMAP: ROS Defense Score (All Cells)")
export_plot_and_table(fig_ros_global, sum_cont(plot_df_umap, 'ROS_Curated_Score'), DIR_GLOBAL, "03_Global_ROS.html", "Global ROS Defense Score")
...
def export_compartment(sub_df, sub_df_umap, prefix, name, score_col, score_name, color_scale):
    ...
    fig_box = px.box(sub_df, x='Day_Clean', y=score_col, color='Infection_Clean', facet_col='Disease_Group', facet_row='majority_voting', points='outliers', color_discrete_map={'Mock': '#1f77b4', 'EBV': '#d62728'})
    ...
    for comp_prefix, (mask, target_score_col, high_label, low_label) in compartments.items():
        ...
        if infected_cells.n_obs > 10:
            ...
            if not top_genes_df.empty:
                ...
                fig_top5.update_yaxes(matches=None, showticklabels=True, categoryorder='total ascending', title="")
...
for target_day, day_int in TARGET_DAYS.items():
    ...
    fig_u2 = px.scatter(day_df_umap, x='UMAP1', y='UMAP2', color='Disease_Group', facet_col='dataset', render_mode='webgl', title=f"{target_day}: Disease Groups", category_orders={"Disease_Group": ["HC", "MS Stable", "MS Active"]})
    ...
    if not wt_day.empty:
        fig_q1_vl = px.box(wt_day, x='majority_voting', y='Plot_Viral_Load', color='Infection_Clean', facet_col='Disease_Group', points='outliers', category_orders={"Disease_Group": ["HC", "MS Stable", "MS Active"]}, color_discrete_map={'Mock': '#1f77b4', 'EBV': '#d62728'}, title=f"{target_day} (WT): Viral Load by Cell Type (Mock vs EBV)")
        ...
        fig_q1_comp = px.bar(comp_df, x='Infection_Clean', y='Percentage', color='majority_voting', facet_col='Disease_Group', category_orders={"Disease_Group": ["HC", "MS Stable", "MS Active"]}, color_discrete_map=CELLTYPE_COLOR_MAP, title=f"{target_day} (WT): Cell Type Shifts")
    ...
    if not enr_inf.empty and ebv_genes_in_data:
        ...
        if not dotplot_df.empty:
            ...
            fig_q2 = px.scatter(dotplot_df, x='EBV_Gene', y='Y_Axis', size='Fraction', color='Mean_Expr', color_continuous_scale='Reds', size_max=15, title=f"{target_day} (Enriched): EBV Pattern")
            fig_q2.update_xaxes(tickangle=-90)
            export_plot_and_table(fig_q2, dotplot_df, DIR_DAYS, f"Q2_Q3_{day_str}_Enriched_EBV_DotPlot.html", f"{target_day} Enriched EBV Patterns", is_umap=False, plot_height=900, bottom_margin=180)
    ...
    fig_q4 = px.bar(q4_comp, x='X_Axis', y='Percentage', color='majority_voting', facet_col='dataset', color_discrete_map=CELLTYPE_COLOR_MAP, title=f"{target_day}: Global Cell Type Predictions")
    fig_q4.update_xaxes(tickangle=-45)
    export_plot_and_table(fig_q4, q4_comp, DIR_DAYS, f"Q4_{day_str}_Global_CellType_Predictions.html", f"{target_day} Cell Type Predictions", is_umap=False, bottom_margin=150)
...
for short_name, exact_cell_type in TARGET_CELLS.items():
    ...
    if not wt_df.empty:
        ...
        def sort_day_disease(row): return (1 if 'HC' in row['Disease_Group'] else (2 if 'Stable' in row['Disease_Group'] else 3), int(re.search(r'\d+', row['Day_Clean']).group()) if re.search(r'\d+', row['Day_Clean']) else 99)
    ...
    # ---> ADDED: Tiered Fallback Logic for Zero Expression <---
    ...
    if not ct_infected.empty and ebv_genes_in_data:
        ...
        if not ct_dot.empty:
            ...
            fig_ct_dot = px.scatter(ct_dot, x='X_Axis', y='EBV_Gene', size='Fraction_of_Cells_Expressing', color='Mean_Expression', color_continuous_scale='Reds', size_max=15, title=f"EBV Genes Expressed in {exact_cell_type} {title_suffix}")
            gene_order = ct_dot.groupby('EBV_Gene')['Mean_Expression'].sum().sort_values(ascending=True).index
            fig_ct_dot.update_yaxes(dtick=1, title="", categoryorder='array', categoryarray=gene_order) 
            fig_ct_dot.update_xaxes(tickangle=-45, categoryorder='array', categoryarray=ct_dot['X_Axis'].unique(), title="")
